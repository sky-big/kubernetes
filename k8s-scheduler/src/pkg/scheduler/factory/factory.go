...
// 根据提供的预选,优选策略以及extender扩展者集合来创建scheduler的config对象
func (f *ConfigFactory) CreateFromKeys(predicateKeys, priorityKeys sets.String, extenders []algorithm.SchedulerExtender) (*scheduler.Config, error) {
	...
	// 根据给定的预选策略名字集合得到对应名字的预选方法集合
	predicateFuncs, err := f.GetPredicates(predicateKeys)
	if err != nil {
		return nil, err
	}

	// 根据给定的优选策略名字集合得到对应名字的优选方法集合
...
// 死循环从空闲pod的队列中获取pod,直到获取一个pod为止
...
// 已经调度成功的pod监听者(用来获得已经调度成功的pod列表)
...
// only include those that have a node name set.
// 得到指定namespace下面已经被成功调度的pod的监听者
...
// 单独定义pod informer(不从工厂模式启动pod informer主要是scheduler组件需要过滤掉不关心的pod)
