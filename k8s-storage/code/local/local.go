...
// 代表在一个节点上一个目录的对象,globalPath字段存储了这个volume对应在节点上的目录路径
// 这个volume的目录将会绑定到每个申请啦这个volume的pod对应的目录下(而pod对应的目录存在于kubelet这个组件的
// 根目录下面，即/var/lib/kubelet目录下面)
...
// 将volume的目录和pod对应的目录进行绑定(每次创建pod的时候如果在pod中挂载了本local volume，则需要到此处来绑定目录)
